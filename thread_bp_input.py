...
class BigPicture_Input_Thread(threading.Thread):
	...
	def __init__(self, callback, libbigpicture=None, period=1, stdin_fn=None): 
	...
	def run(self):
		...
		if sys.stdout.isatty():
			...
			if self.libbigpicture == None:
				...
				while not self.stop_input_thread and not self.libbigpicture.LibExt.getTTF(self.libbigpicture.global_bigpicture):
					#print("Enter commands:")
					i, o, e = select.select( [self.stdin], [], [], self.period )
					if (i):
						command = self.stdin.readline().strip()

...
class JB468Multi( scenario.Scenario ):
	"""The Joshua Blackwood 468 Scenario, with multiple colors"""
	...
	def __init__( self, puzzle, params={} ):
		...
		self.heuristic_patterns = [ [ 9, 12, 15 ], [ 10, 4, 7 ] ]
	...
	def prepare_patterns_count_heuristics( self ):
		...
		for i in range(self.puzzle.board_wh):
			if i in range(17, 27):
				self.heuristic_patterns_count[1][i] = int((i - 16) * 3.1)
			elif i in range(27, 57):
				self.heuristic_patterns_count[1][i] = int((i - 26) * 1.43333) + 31
			elif i in range(57, 97):
				self.heuristic_patterns_count[1][i] = int((i - 56) * 0.65) + 74
			elif i in range(97, 157):
				self.heuristic_patterns_count[1][i] = int((i - 96) / 3.75) + 100
...
scenario.global_list.append(JB468Multi)

...
class JB468Stats16( scenario.Scenario ):
	...
	def prepare_stats16_count_heuristics( self ):

		total_weight = sum(self.puzzle.pieces_stats16_weight)
		print(total_weight)
		pente = total_weight/(self.puzzle.board_wh-157-16)
		#print(pente)
		
		for i in range(self.puzzle.board_wh):
			if i in range(157, self.puzzle.board_wh-16):
				self.heuristic_stats16_count[i] = int((i) * pente)
			#elif i in range(224, 240):
			#	self.heuristic_stats16_count[i] = int((i - 96) / 3.75) + 0
...
scenario.global_list.append(JB468Stats16)
